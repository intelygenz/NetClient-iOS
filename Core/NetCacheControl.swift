...
extension NetCacheControl: RawRepresentable {
    ...
    public init(rawValue: RawValue) {
        if rawValue.hasPrefix(StringValue.maxAge), let timeInterval = TimeInterval(rawValue.substring(from: rawValue.index(StringValue.maxAge.endIndex, offsetBy: 2))) {
        ...
        } else if rawValue.hasPrefix(StringValue.minFresh), let timeInterval = TimeInterval(rawValue.substring(from: rawValue.index(StringValue.minFresh.endIndex, offsetBy: 2))) {
